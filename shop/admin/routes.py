...
def register():
    ...
    if form.validate_on_submit():  # validate the user's input into the form
        password_errors = validate_password(
            form.password.data
        )  # run the validate_password function above
        ...
        ).first()  # check if the username is already used
        ...
        if form.password.data.strip() != form.confirm.data.strip():
            flash(
                "The passwords do not match. Please re-enter the passwords.", "danger"
            )
            return render_template("pages/register.html", form=form)

        hash_password = bcrypt.generate_password_hash(form.password.data).decode(
            "utf-8"
        )  # hash and decode to string
        ...
        )  # show the user their accunt was created
...
def login():
    ...
    if form.validate_on_submit():
        user = User.query.filter_by(
            username=form.username.data
        ).first()  # check in the database if the user's entered credentials match
...
def admin_register():
    ...
    if form.validate_on_submit():
        password_errors = validate_password(
            form.password.data
        )  # use the password_validation function
        ...
        existing_admin = Admin.query.filter_by(
            username=form.username.data
        ).first()  # use the database to check if the username the admin has chosen is taken
...
def admin_login():
    ...
    if form.validate_on_submit():
        admin = Admin.query.filter_by(
            username=form.username.data
        ).first()  # check in the database if the admin's entered credentials match
        ...
        if admin and bcrypt.check_password_hash(admin.password, form.password.data):
            ...
            )  # Or any page you want to redirect to
        else:
            # create session values for the user
...
def admin_dashboard():
    if "id" not in session:  # if the user hasn't logged in
        ...
        )  # display to the user that they need to login
        return redirect(url_for("home"))
        ...
        orders = Order.query.join(
            User
        ).all()  # gather all of the orders from the database
        order_items = (
            OrderItem.query.all()
        )  # gather all of the items ordered from the database
        active_users = (
            User.query.all()
        )  # gather all of the users that have registered from the database
...
def add_product():
    if "id" not in session:  # if the admin hasn't logged in
        flash(
            "Admin access required.", "danger"
        )  # display to the admin that they need to login
        return redirect(url_for("home"))
    else:  # otherwise if the admin has logged in
        form = (
            ProductForm()
        )  # take the information that the admin submitted on the form
...
def delete_product(product_id):
    if "id" not in session:  # if the admin hasn't logged in
        return (
            ...
            403,
        )  # return a JSON error with 403 status code
...
def all_products():
    if "id" not in session:  # if the user hasn't logged in
        flash(
            "Please create an account with SecureCart first.", "danger"
        )  # display to the user that they need to login
        return redirect(url_for("home"))
    else:  # otherwise if the user has logged in
...
def add_to_cart(product_id):
    if "id" not in session:  # if the user hasn't logged in
        ...
        )  # get the quantity of the product the user wants
                ...
                cart_items.append(
                    {
                        ...
                        * quantity,  # calculate subtotal for the product
                    }
                )
...
def create_payment_intent():
    if "id" not in session:  # if the user hasn't logged in
        ...
        total_price = request.json.get(
            "total_price"
        )  # get the total price from the webpage
    ...
    if "id" not in session:  # if the user hasn't logged in
        ...
        if "id" in session:  # check if the user is logged in
            if "cart" in session:
                ...
                product_ids = [
                    int(pid) for pid in cart.keys()
                ]  # convert cart keys (product ids) to integers
                ...
                for product in products:
                    product_id_str = str(
                        product.id
                    )  # make sure product id is a string (to match session keys)
                    quantity = cart.get(
                        product_id_str, 0
                    )  # get how many of this product are in the cart (default to 0)
                    ...
                    cart_items.append(
                        {
                            ...
                            "quantity": quantity,
                            "subtotal": product.price
                            * quantity,  # subtotal for this product (price * quantity)
                        }
                    )
